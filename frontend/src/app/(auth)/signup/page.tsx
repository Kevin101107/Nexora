...
export default function SignupPage() {
  ...
  if (done) {
    return (
      <div className="relative min-h-screen overflow-hidden bg-[#f7f5f0] dark:bg-[#08080f] flex items-center justify-center px-4">
        {/* Background blobs */}
          ...
          <div className="glass-card rounded-3xl p-8">
            <form onSubmit={handleSignup} className="space-y-5">
              ...
              <div>
                ...
                <input value={name} onChange={(e) => setName(e.target.value)} className="input-field !py-3.5 !rounded-xl !text-[15px]" placeholder="Your name" autoFocus />
              </div>
              <div>
                <label className="label">Email</label>
                <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required className="input-field !py-3.5 !rounded-xl !text-[15px]" placeholder="you@example.com" />
              </div>
              <div>
                <label className="label">Password</label>
                <div className="relative">
                  <input type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} required minLength={6} className="input-field !py-3.5 !rounded-xl !text-[15px] !pr-11" placeholder="At least 6 characters" />
                  <button type="button" onClick={() => setShowPassword((v) => !v)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 dark:text-white/25 hover:text-gray-600 dark:hover:text-white/50 transition-colors p-1" tabIndex={-1}>
